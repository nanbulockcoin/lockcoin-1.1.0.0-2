...
const string strMessageMagic = "LockCoin Signed Message:\n";
 ...
 *
 *
 * BLOCK NO. 1-1,500 - 1,500 - 25 bonus blocks payout x5 (7,500) - 1 every hour					- 1/3600    - 1 in 60 pow blocks
 * BLOCK NO. 1,501-5,000 - 750 - 30 bonus blocks payout x10 (7,500) - 1 every 1.9 hours			- 1/6840    - 1 in 114 pow blocks
 * BLOCK NO. 5,001-10,000 - 600 - 35 bonus blocks payout x20 (12,000) - 1 every 2.4 hours		- 1/8640    - 1 in 144 pow blocks
 * BLOCK NO. 10,001-15,000 - 500 - 40 bonus blocks payout x40 (20,000) - 1 every 2.1 hours		- 1/7560    - 1 in 126 pow blocks
 * BLOCK NO. 15,001-20,000 - 250 - 50 bonus blocks payout x100 (25,000) - 1 every 1.7 hours		- 1/6120    - 1 in 102 pow blocks
 *
 */
int64_t GetProofOfWorkBonusRewardFactor(CBlockIndex* pindex)
{
    ...
    if ( nBlockHeight > 0 && nBlockHeight < 2500){ // hardfork at block 2500
        random = generateMTRandom(seed, 60000);
        if ( SEED_DEBUG ) printf(">> cseed = %s, random = %d\n", cseed, random);
        return random>30000 && random<31001?5:0;     // 1 in 60, x5
    }else if( nBlockHeight < 5000 ){
        random = generateMTRandom(seed, 114000);
        if ( SEED_DEBUG ) printf(">> cseed = %s, random = %d\n", cseed, random);
        return random>56500 && random<57501?10:0;   // 1 in 115, x10
    }else if ( nBlockHeight < 10000 ){
        random = generateMTRandom(seed, 144000);
        if ( SEED_DEBUG ) printf(">> cseed = %s, random = %d\n", cseed, random);
        return random>71500 && random<72501?20:0;   // 1 in 144, x20
    }else if ( nBlockHeight < 15000 ){
        random = generateMTRandom(seed, 126000);
        if ( SEED_DEBUG ) printf(">> cseed = %s, random = %d\n", cseed, random);
        return random>62500 && random<63501?40:0;   // 1 in 126, x40
    }else if ( nBlockHeight < LAST_POW_BLOCK ){
        random = generateMTRandom(seed, 102000);
        if ( SEED_DEBUG ) printf(">> cseed = %s, random = %d\n", cseed, random);
...
static const int YEARLY_POS_BLOCK_COUNT = 1049040;

// Bonus POS block first 12 months: 1 chance in 2.5 million coin-day (up to 5 times per day) a bonus-PoS-block of 512 coins on top of your regular PoS will be generated.

int64_t GetProofOfStakeReward(int64_t nCoinAge, const CBlockIndex* pindex)
{
    int64_t nRewardCoinYear = MAX_PROOF_OF_STAKE_STABLE; // 0.01 * COIN
	...
	if(nPoSHeight < YEARLY_POS_BLOCK_COUNT)	
	{
        ...
        const char* cseed = cseed_str.c_str();
        long seed = hex2long(cseed);
        int random = generateMTRandom(seed, 5000000);

        int64_t delta = nCoinAge;

        //
        int lowerLimit = 2500000 - delta;
        int upperLimit = 2500000 + delta;

        if(lowerLimit < 0)
            lowerLimit = 0;

        if(upperLimit > 5000000)
            upperLimit = 5000000;


        if ( SEED_DEBUG ) printf(">> random = %d, lowerLimit = %d, upperLimit = %d, nPoWHeight = %d, nCoinAge = %d \n",  random, lowerLimit, upperLimit, nPoWHeight, nCoinAge);

        if(random >= lowerLimit && random <= upperLimit)
        {
            if ( SEED_DEBUG ) printf(">> Got Super-PoS-Block...yay!\n");
            nSubsidy += 512 * COIN;
        }
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "LockCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Ebola virus is spreading";
        CTransaction txNew;
        txNew.nTime = 1404473470;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1404473470;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 672245938;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x6d20aebcdfe15d109a0deb3595d2ae155ea23b0c7618abc1754e4b678f0bba77"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xdd, 0xab, 0xba, 0xcc };
